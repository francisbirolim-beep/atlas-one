...
export async function listarProdutosTecnicos(): Promise<Produto[]> {
  const { data } = await supabase.from('produtos').select('*').eq('ativo', true).in('categoria', ['perfil','acessorio','vidro','outro']).not('unidade', 'is', null).neq('unidade', '').order('categoria').order('nome')
