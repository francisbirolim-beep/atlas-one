...
export async function criarOrcamentoNoServidor(dados: DadosOrcamentoForm): Promise<{ ok: boolean; id?: string; error?: string }> {
  ...
  for (const it of itens) {
    ...
    if (tipoMedidaItem === 'final') {
      ...
      itensSalvos.push({
        id: it.id, item_tipo: itemTipo, material_categoria: it.materialCategoria || null, material_unidade: it.materialUnidade || null, produto_nome: it.produtoNome || null, ambiente: it.ambiente?.trim() || null, tipo_esquadria: it.tipo as TipoEsquadria,
