...
function extrairCnpjEmitente(texto: string) {
  ...
  for (const m of Array.from(topo.matchAll(rotulo))) {
    const inicio = (m.index || 0) + (m[0]?.length || 0)
    const trecho = topo.slice(inicio, inicio + 140)
    const candidato = trecho.match(/(?:\d[\s.\/.\-]?){14}/)?.[0] || ''
    const digitos = somenteDigitos(candidato)
    if (digitos.length === 14) return digitos
  }
  const primeiroFormatado = (topo.match(/\b\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}\b/g) || [])[0] || ''
  return primeiroFormatado ? somenteDigitos(primeiroFormatado) : ''
}
...
function parseLinhaProduto(linha: string): NFeItemNormalizado | null {
  ...
  for (let i = cfopIndex + 2; i < tokens.length && numeros.length < 3; i += 1) {
    ...
    if (tokenNumero(token)) numeros.push(token)
  ...
  return {
    ...
    valorUnitario,
    valorTotal,
    dadosOrigem: { origemLeitura: 'pdf_danfe_v4', linha: normalizada.slice(0, 1800) },
  }
}
...
export async function lerPdfDanfeV4(buffer: Buffer): Promise<NFeNormalizada> {
  ...
  else avisos.push('O PDF/DANFE ainda não permitiu identificar automaticamente a tabela de itens. O diagnóstico do servidor será usado para ajustar este modelo de DANFE.')
