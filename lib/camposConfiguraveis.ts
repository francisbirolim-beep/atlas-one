...
const CAMPOS_PADRAO: CampoConfiguravel[] = [
  ...
  },
  {
    ...
    ordem: 130,
    obrigatorioEm: [],
    mostrarEm: ['cliente', 'orcamento', 'contrato'],
  },
  {
    id: 'forma_pagamento',
    ...
    ativo: true,
    ordem: 140,
    opcoes: ['Pix', 'Boleto', 'Cartão', 'Cheque', 'Financiamento', 'Dinheiro', 'Outro'],
  ...
  },
  {
    ...
    ativo: true,
    ordem: 150,
    placeholder: 'Ex.: 50% entrada + 50% instalação',
    obrigatorioEm: ['confirmacao_venda', 'contrato', 'financeiro'],
    mostrarEm: ['confirmacao_venda', 'orcamento', 'contrato', 'financeiro'],
  },
]
...
function mesclarComPadrao(camposSalvos: CampoConfiguravel[]): CampoConfiguravel[] {
  ...
  for (const padrao of CAMPOS_PADRAO) {
    if (!porChave.has(padrao.chave)) mesclados.push(padrao)
  }
...
export function campoNoContexto(
  campos: CampoConfiguravel[],
  chave: string,
  contexto: ContextoCampoConfiguravel
): CampoConfiguravel | undefined {
...
export const CONTEXTOS_CAMPO: { valor: ContextoCampoConfiguravel; label: string }[] = [
  ...
  { valor: 'confirmacao_venda', label: 'Confirmação de venda' },
  { valor: 'orcamento', label: 'Orçamento' },
  { valor: 'contrato', label: 'Contrato' },
  { valor: 'medicao_final', label: 'Medição Final' },
