...
const ROTULOS_CATEGORIA: Record<string, string> = {
  porta: 'Portas', janela: 'Janelas', modulo_fixo: 'Módulos Fixos', fachada: 'Fachadas',
  box: 'Boxes', painel_ripado: 'Painéis / Ripados', acm: 'ACM', cobertura_claraboia: 'Coberturas',
  contramarco_arremate: 'Contramarcos / Arremates', espelho: 'Espelhos', portao_grade: 'Portões / Grades',
  guarda_corpo_corrimao: 'Guarda-corpos / Corrimãos', vidro: 'Vidros', tela_mosquiteira: 'Telas Mosquiteiras', outros: 'Outros',
}
...
export default function OrcamentoSobMedidaBuilder() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      supabase.from('clientes').select('id,nome,telefone,whatsapp,cidade').ilike('nome', `%${termo}%`).order('nome').limit(8).then(({ data }) => {
  ...
  const filtradas = useMemo(() => {
    ...
    const linha = linhas.find(l => l.id === linhaFiltro)
  ...
  function adicionar(t: Tipologia) {
    ...
    setItens(prev => [...prev, {
      ...
      linhaId: linhaPadraoId || linhaCompativel?.id || '',
      cor: corPadrao,
  ...
  function salvarPreview() {
    ...
    window.sessionStorage.setItem('atlas_orcamento_sob_medida_builder_v1', JSON.stringify({
      ...
      cidade,
      temperatura,
      padroes: { linhaId: linhaPadraoId, cor: corPadrao, contramarco: contramarcoPadrao, vidro: vidroPadrao, arremate: arrematePadrao },
      itens,
    }))
    setSalvo(true)
  }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-[1500px] items-center gap-3 px-4 py-4 lg:px-6">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-slate-500">Escolha o cliente, defina os padrões e adicione todas as tipologias antes de configurar cada peça.</p>
          </div>
      ...
      <main className="mx-auto grid max-w-[1500px] gap-5 px-4 py-5 pb-24 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-6">
        <section className="min-w-0 space-y-5">
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <div className="relative md:col-span-2">
                ...
                <div className="relative">
                  ...
                  <input value={buscaCliente} onChange={e => { setBuscaCliente(e.target.value); if (cliente && e.target.value !== cliente.nome) setCliente(null) }} placeholder="Digite pelo menos 3 letras..." className="w-full rounded-xl border border-slate-300 py-2.5 pl-9 pr-9 text-sm outline-none focus:border-blue-500"/>
                  {cliente && <button onClick={() => { setCliente(null); setBuscaCliente('') }} className="absolute right-3 top-2.5 text-slate-400"><X size={18}/></button>}
                </div>
                {clientesEncontrados.length > 0 && <div className="absolute z-30 mt-1 w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">{clientesEncontrados.map(c => <button key={c.id} onClick={() => selecionarCliente(c)} className="block w-full border-b border-slate-100 px-3 py-2.5 text-left last:border-0 hover:bg-blue-50"><span className="block text-sm font-semibold">{c.nome}</span><span className="text-xs text-slate-500">{c.cidade || 'Cidade não informada'} · {c.whatsapp || c.telefone || 'Sem telefone'}</span></button>)}</div>}
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-600">Cidade</label>
                <input value={cidade} onChange={e => setCidade(e.target.value)} placeholder="Cidade da obra" className="w-full rounded-xl border border-slate-300 px-3 py-2.5 text-sm"/>
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-600">Temperatura</label>
                <select value={temperatura} onChange={e => setTemperatura(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm"><option value="">Selecione</option><option value="quente">Quente</option><option value="morno">Morno</option><option value="frio">Frio</option></select>
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            <div className="mb-4 flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-600 text-sm font-bold text-white">3</span><div><h2 className="font-bold">Adicionar tipologias</h2><p className="text-xs text-slate-500">Clique em + para incluir quantas peças forem necessárias no mesmo orçamento.</p></div></div>
            <div className="grid gap-2 sm:grid-cols-3">
              <div className="relative sm:col-span-1"><Search size={16} className="absolute left-3 top-3 text-slate-400"/><input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Pesquisar tipologia..." className="w-full rounded-xl border border-slate-300 py-2.5 pl-9 pr-3 text-sm"/></div>
        ...
        <aside className="min-w-0 lg:sticky lg:top-4 lg:self-start">
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="max-h-[62vh] space-y-3 overflow-y-auto p-4">
              ...
              {itens.map((item, index) => <div key={item.uid} className="rounded-xl border border-slate-200 p-3"><div className="mb-3 flex items-start gap-2"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-bold text-slate-600">{index + 1}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{item.nome}</p><p className="text-[11px] text-slate-500">Cada peça pode ter variáveis próprias.</p></div><button onClick={() => setItens(prev => prev.filter(i => i.uid !== item.uid))} className="rounded-lg p-1.5 text-red-500 hover:bg-red-50"><Trash2 size={16}/></button></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2"><select value={item.linhaId} onChange={e => atualizarItem(item.uid, { linhaId: e.target.value })} className="min-w-0 rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs"><option value="">Linha</option>{linhasDoItem(item).map(l => <option key={l.id} value={l.id}>{l.nome}</option>)}</select><select value={item.cor} onChange={e => atualizarItem(item.uid, { cor: e.target.value })} className="min-w-0 rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs"><option value="preto">Preto</option><option value="branco">Branco</option><option value="madeirado">Amadeirado</option><option value="outro">Outra cor</option></select><select value={item.contramarco} onChange={e => atualizarItem(item.uid, { contramarco: e.target.value })} className="min-w-0 rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs"><option value="sim">Com contramarco</option><option value="nao">Sem contramarco</option></select><input value={item.vidro} onChange={e => atualizarItem(item.uid, { vidro: e.target.value })} placeholder="Vidro" className="min-w-0 rounded-lg border border-slate-200 px-2 py-2 text-xs"/><select value={item.arremate} onChange={e => atualizarItem(item.uid, { arremate: e.target.value })} className="min-w-0 rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs"><option value="padrao">Arremate padrão</option><option value="sim">Com arremate</option><option value="nao">Sem arremate</option></select><input type="number" min={1} value={item.quantidade} onChange={e => atualizarItem(item.uid, { quantidade: Math.max(1, Number(e.target.value) || 1) })} className="min-w-0 rounded-lg border border-slate-200 px-2 py-2 text-xs"/></div></div>)}
            </div>
