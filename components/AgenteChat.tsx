...
import { Sparkles, X, Send, Loader2, Check, Ban } from 'lucide-react'
...
export default function AgenteChat() {
  ...
  async function enviar() {
    ...
    setErro('')
    setEntrada('')
    setBolhas((prev) => [...prev, { papel: 'user', texto }])
    setCarregando(true)
    try {
      const resp = await chamar('/api/agente/chat', { mensagem: texto, messages: historico })
        ...
        <div className="fixed bottom-24 right-6 z-50 flex h-[520px] max-h-[75vh] w-[360px] max-w-[90vw] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-slate-200">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
            {bolhas.length === 0 && (
              <p className="text-sm text-slate-400">
                Oi, {usuario.nome.split(' ')[0]}! Pode perguntar sobre tarefas, orcamentos, clientes, assistencias ou pedir para eu criar algo.
              </p>
            )}
            {bolhas.map((b, i) => (
          ...
          <div className="flex items-center gap-2 border-t border-slate-100 p-3">
            ...
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-full bg-[#182444] text-white disabled:opacity-40"
            >
              <Send size={16} />
            </button>
          </div>
