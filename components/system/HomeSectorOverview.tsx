...
type Dados = { obras: number; medicoes: number; producao: number; receber: number }

export default function HomeSectorOverview({ dashboard }: Props) {
  const [dados, setDados] = useState<Dados>({ obras: 0, medicoes: 0, producao: 0, receber: 0 })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('financeiro_contas_receber').select('valor,valor_pago,status'),
      ...
      const receber = (contas.data || []).filter(c => c.status !== 'cancelado').reduce((s,c) => s + Math.max(0, Number(c.valor || 0) - Number(c.valor_pago || 0)), 0)
      setDados({
  ...
  const base = dashboard === 'engenharia'
    ...
    : dashboard === 'producao'
      ...
      : dashboard === 'instalacao'
        ...
        : [
            { label:'A receber', value:moeda(dados.receber), href:'/financeiro', icon:CircleDollarSign },
