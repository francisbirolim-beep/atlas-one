...
type Dados = { obras: number; medicoes: number; producao: number; receber: number; vencido: number; contasAbertas: number; etapasProducao: { nome:string; total:number }[]; etapasEngenharia: { nome:string; total:number }[]; etapasInstalacao: { nome:string; total:number }[] }

export default function HomeSectorOverview({ dashboard }: Props) {
  const [dados, setDados] = useState<Dados>({ obras: 0, medicoes: 0, producao: 0, receber: 0, vencido: 0, contasAbertas: 0, etapasProducao: [], etapasEngenharia: [], etapasInstalacao: [] })
  ...
  useEffect(() => {
    ...
    ,
      ...
      supabase.from('setor_kanban_colunas').select('id,nome,ordem').eq('setor_id','instalacao').order('ordem',{ascending:true}),
      supabase.from('setor_kanban_itens').select('id,coluna_id').eq('setor_id','instalacao'),
    ]).then(([obras, medicoes, producao, contas, colunas, colunasEng, itensEng, colunasInst, itensInst]) => {
      if (!ativo) return
      ...
      setDados({
        ...
        etapasInstalacao: (colunasInst.data || []).map(coluna => ({ nome: coluna.nome, total: (itensInst.data || []).filter(item => item.coluna_id === coluna.id).length })),
      })
      setCarregando(false)
    })
    return () => { ativo = false }
