...
const TITULOS: { prefixo: string; titulo: string; grupo: string }[] = [
  ...
  { prefixo: '/prospeccao', titulo: 'Prospecção em Campo', grupo: 'Comercial' },
  ...
  { prefixo: '/producao/medicao-final', titulo: 'Medição Final', grupo: 'Produção' },
  { prefixo: '/producao/plano-corte', titulo: 'Plano de Corte', grupo: 'Produção' },
...
const PAGINAS: { id: string; titulo: string; subtitulo: string; href: string; termos: string[] }[] = [
  ...
  { id: 'pagina-prospeccao', titulo: 'Prospecção', subtitulo: 'Página · Obras e contatos em campo', href: '/prospeccao', termos: ['prospeccao', 'prospecção', 'obra', 'visita', 'vendedor'] },
          ...
          <div className="relative flex flex-none items-center justify-end gap-1.5 sm:flex-1 sm:gap-2 xl:max-w-[430px]">
            ...
            {perfilAberto && (
              <div className="absolute right-0 top-12 z-50 w-56 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
                {usuario?.role === 'master' && <Link href="/configuracoes" onClick={() => setPerfilAberto(false)} className="flex items-center gap-2 px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50"><Settings size={16}/>Configurações</Link>}
                <button type="button" onClick={() => void sair()} className="flex w-full items-center gap-2 border-t border-slate-100 px-4 py-3 text-left text-sm font-medium text-red-600 hover:bg-red-50"><LogOut size={16}/>Sair</button>
              </div>
            )}
          </div>
        </div>
