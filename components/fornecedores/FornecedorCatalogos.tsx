...
import { FileText, Loader2, Upload, Bot, PackageSearch } from "lucide-react";
...
const statusLabel: Record<string,string> = { vinculado:"Já existe no Atlas", criado_pendente:"Novo produto — validar", revisar:"Precisa revisar" };
...
export default function FornecedorCatalogos({ fornecedorId }: Props) {
  ...
  const itensVisiveis=useMemo(()=>itens.filter(i=>!filtroDoc||i.documento_id===filtroDoc),[itens,filtroDoc]);
  ...
  return <section className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
    ...
    <div className="grid grid-cols-3 gap-2"><div className="rounded-xl bg-slate-50 p-3 text-center"><b className="text-lg">{vinculos.length}</b><p className="text-[11px] text-slate-500">Produtos vinculados</p></div><div className="rounded-xl bg-slate-50 p-3 text-center"><b className="text-lg">{resumo.criados}</b><p className="text-[11px] text-slate-500">Novos para validar</p></div><div className="rounded-xl bg-slate-50 p-3 text-center"><b className="text-lg">{resumo.revisar}</b><p className="text-[11px] text-slate-500">Com dúvida</p></div></div>
