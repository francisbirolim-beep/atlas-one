...
export default function ComprasPage(){
 ...
 return <main className="min-h-screen w-full min-w-0 overflow-x-hidden bg-slate-50 p-4 sm:p-6"><div className="mx-auto w-full min-w-0 max-w-7xl space-y-5">
  <header className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[.14em] text-slate-400">Operações • Compras</p><h1 className="mt-1 text-2xl font-bold text-slate-900">Central de Compras</h1><p className="mt-1 break-words text-sm leading-6 text-slate-600">NF → vínculo/cadastro → financeiro → recebimento → estoque, com rastreabilidade do arquivo original.</p></header>
...
function ResumoCard({label,valor,icon,destaque=false}:{label:string;valor?:number;icon:React.ReactNode;destaque?:boolean}){return <div className={`min-w-0 rounded-2xl border bg-white p-5 shadow-sm ${destaque?'border-amber-300':'border-slate-200'}`}><div className="flex min-w-0 items-center justify-between gap-3 text-slate-500"><span className="min-w-0 break-words text-sm font-medium">{label}</span><span className="shrink-0">{icon}</span></div><div className="mt-2 text-3xl font-bold">{valor??'—'}</div></div>}
function Acao({href,icon,titulo,descricao,destaque=false}:{href:string;icon:React.ReactNode;titulo:string;descricao:string;destaque?:boolean}){return <Link href={href} className={`min-w-0 rounded-2xl border bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${destaque?'border-amber-300':'border-slate-200'}`}><div className="text-slate-700">{icon}</div><h2 className="mt-4 break-words font-bold">{titulo}</h2><p className="mt-1 break-words text-sm leading-6 text-slate-600">{descricao}</p></Link>}
