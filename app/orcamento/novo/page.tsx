...
export default function NovoOrcamento() {
  ...
  useEffect(() => {
    supabase.from('clientes').select('id,nome,apelido,whatsapp,telefone,cidade,bairro,cpf_cnpj').order('nome').limit(1000).then(({ data }) => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 to-brand-navyLight">
    ...
    <main className="mx-auto max-w-2xl px-4 py-8"><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-brand-navy">Novo atendimento</p><h2 className="mt-2 text-xl font-bold text-slate-900">Qual é o nome do cliente?</h2><p className="mt-2 text-sm leading-6 text-slate-500">Pesquise primeiro. Encontrando o cliente, abra a ficha dele para escolher orçamento sob medida, venda balcão, assistência ou manutenção. Se não existir, crie apenas com nome e sobrenome.</p><div className="relative mt-5"><Search size={18} className="absolute left-3 top-3.5 text-slate-400"/><input autoFocus value={busca} onChange={e => { setBusca(e.target.value); setErro('') }} placeholder="Digite nome e sobrenome..." className="w-full rounded-xl border border-slate-300 py-3 pl-10 pr-3 text-base outline-none transition focus:border-brand-navy focus:ring-2 focus:ring-brand-navy/10"/></div>{carregando ? <div className="mt-4 flex items-center gap-2 text-sm text-slate-500"><Loader2 size={16} className="animate-spin"/> Carregando clientes...</div> : null}{encontrados.length > 0 ? <div className="mt-3 overflow-hidden rounded-xl border border-slate-200">{encontrados.map(cliente => <button key={cliente.id} type="button" onClick={() => router.push(`/clientes/${cliente.id}`)} className="flex w-full items-center gap-3 border-b border-slate-100 px-4 py-3 text-left transition last:border-b-0 hover:bg-slate-50"><span className="rounded-lg bg-emerald-50 p-2 text-emerald-700"><UserCheck size={16}/></span><span className="min-w-0 flex-1"><strong className="block text-sm text-slate-800">{cliente.nome}</strong><span className="mt-0.5 block truncate text-xs text-slate-500">{[cliente.cidade, cliente.bairro, cliente.whatsapp || cliente.telefone, cliente.cpf_cnpj].filter(Boolean).join(' • ') || 'Abrir Cliente 360'}</span></span><span className="text-xs font-semibold text-brand-navy">Abrir</span></button>)}</div> : null}{busca.trim().length >= 2 && encontrados.length === 0 && !carregando ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-semibold text-amber-900">Cliente ainda não cadastrado</p><p className="mt-1 text-xs leading-5 text-amber-800">Vamos criar o Cliente 360 agora. Os demais dados poderão ser completados quando a venda for confirmada.</p><button type="button" disabled={salvando} onClick={cadastrarEAbrir} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-brand-navy px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">{salvando ? <Loader2 size={16} className="animate-spin"/> : <UserPlus size={16}/>}Cadastrar e abrir Cliente 360</button></div> : null}{erro ? <p className="mt-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{erro}</p> : null}</section></main>
  </div>
}
