...
import { listarConversasAtendimento, buscarClienteAtendimento, type AtendimentoConversa, type AtendimentoCliente } from '@/lib/atendimento'
...
export default function AtendimentoPage(){
 ...
 return <main className="min-h-screen bg-slate-100 p-3"><div className="mx-auto max-w-5xl overflow-hidden rounded-2xl border bg-white shadow-sm">
  <header className="border-b p-4"><h1 className="text-xl font-bold">Central de Conversas</h1><p className="text-sm text-slate-500">Teste 6 · conversas reais</p></header>
  <div className="p-4"><input value={busca} onChange={e=>setBusca(e.target.value)} placeholder="Buscar conversas..." className="w-full rounded-xl border px-4 py-3 text-sm outline-none"/>
   <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4"><b className="text-emerald-900">Conversas carregadas: {conversas.length}</b>{erro&&<p className="mt-2 break-words text-sm text-red-700">{erro}</p>}</div>
