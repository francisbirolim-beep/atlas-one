...
import { ArrowLeft, Search, Phone, Mail, MapPin, Send, UserRoundCheck, MessageCircle, UsersRound } from 'lucide-react'
...
export default function AtendimentoPage(){
 ...
 return <main className="min-h-screen bg-slate-100 p-2 md:p-4"><div className="mx-auto max-w-[1600px] overflow-hidden rounded-2xl border bg-white shadow-sm">
  <header className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3">
   <div className="flex items-center gap-3"><Link href="/" className="rounded-lg p-2 hover:bg-slate-100"><ArrowLeft size={19}/></Link><div><div className="flex items-center gap-2"><h1 className="text-xl font-bold">Central de Conversas</h1><span className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700"><MessageCircle size={13}/> WhatsApp</span></div><p className="text-xs text-slate-500">WhatsApp da Esquadrifácio · {conversas.length} conversa(s)</p></div></div>
   <div className="flex items-center gap-2"><span className="hidden items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-500 md:inline-flex"><UsersRound size={14}/> Grupos em preparação</span><span className="text-sm text-emerald-600">● Online</span></div>
  </header>
  ...
  <div className="grid min-h-[680px] lg:grid-cols-[310px_1fr_350px]">
   ...
   <section className="flex min-w-0 flex-col bg-[#efeae2]">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-[#f0f2f5] p-4"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-full bg-emerald-100 font-bold">{iniciais(cliente?.nome||ativa.telefone)}</span><div><h2 className="font-bold">{cliente?.nome||ativa.telefone}</h2><p className="text-xs text-slate-500">{ativa.telefone} · {cliente?<span className="text-emerald-600">Cliente cadastrado</span>:'Não vinculado ao Cliente 360'}</p></div></div><div className="flex flex-wrap gap-2">{ativa.status==='aguardando'&&<button onClick={assumir} disabled={salvando} className="rounded-xl bg-emerald-600 px-3 py-2 text-sm font-bold text-white">Assumir atendimento</button>}<select value={rotuloStatus[ativa.status]||'Em atendimento'} onChange={e=>void mudarStatus(e.target.value)} className="rounded-xl bg-blue-600 px-3 py-2 text-sm font-bold text-white"><option>Em atendimento</option><option>Aguardando cliente</option><option>Transferido</option><option>Finalizado</option></select></div></div>
