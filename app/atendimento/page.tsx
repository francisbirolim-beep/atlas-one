...
export default function AtendimentoPage() {
  ...
  useEffect(()=>{let ativo=true;(async()=>{try{const {count,error}=await supabase.from('atendimento_conversas').select('id',{count:'exact',head:true});if(!ativo)return;setStatus(error?'Supabase respondeu com erro controlado: '+error.message:'Supabase conectado · '+(count??0)+' conversa(s)')}catch(e){if(ativo)setStatus('Falha controlada: '+String(e))}})();return()=>{ativo=false}},[])
