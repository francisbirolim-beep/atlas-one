 ...
 return <main className="min-h-screen bg-slate-100 p-2 md:p-4"><div className="mx-auto max-w-[1600px] overflow-hidden rounded-2xl border bg-white shadow-sm">
  ...
  <div className="grid min-h-[680px] lg:grid-cols-[310px_1fr_350px]">
   ...
   <section className={"min-w-0 flex-col bg-[#f6f3ee] "+(conversaMobile?"flex":"hidden lg:flex")}>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-white p-4"><div className="flex items-center gap-3"><button onClick={()=>setConversaMobile(false)} className="rounded-full p-2 hover:bg-slate-100 lg:hidden" aria-label="Voltar às conversas"><ArrowLeft size={20}/></button><span className="grid h-11 w-11 place-items-center rounded-full bg-emerald-100 font-bold">{iniciais(cliente?.nome||ativa.telefone)}</span><div><h2 className="font-bold">{cliente?.nome||ativa.telefone}</h2><p className="text-xs text-slate-500">{ativa.telefone} · {cliente?<span className="text-emerald-600">Cliente cadastrado</span>:'Não vinculado ao Cliente 360'}</p></div></div><div className="flex flex-wrap gap-2">{ativa.status==='aguardando'&&<button onClick={assumir} disabled={salvando} className="rounded-xl bg-emerald-600 px-3 py-2 text-sm font-bold text-white">Assumir atendimento</button>}<select value={rotuloStatus[ativa.status]||'Em atendimento'} onChange={e=>void mudarStatus(e.target.value)} className="rounded-xl bg-blue-600 px-3 py-2 text-sm font-bold text-white"><option>Em atendimento</option><option>Aguardando cliente</option><option>Transferido</option><option>Finalizado</option></select></div></div>
