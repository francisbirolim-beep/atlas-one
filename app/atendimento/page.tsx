...
export default function AtendimentoPage(){
 ...
 useEffect(()=>{const conversa=conversas[0];if(!conversa)return;let ativo=true;listarMensagensAtendimento(conversa.id).then(m=>{if(ativo)setMensagens(m)}).catch(e=>{if(ativo)setErro(String(e))});const parar=observarAtendimento(()=>{setRt('Segundo Realtime recebeu atualização')});setRt('Segundo Realtime criado');return()=>{ativo=false;parar()}},[conversas[0]?.id])
 ...
 return <main className="min-h-screen bg-slate-100 p-3"><div className="mx-auto max-w-5xl overflow-hidden rounded-2xl border bg-white shadow-sm">
  <header className="border-b p-4"><h1 className="text-xl font-bold">Central de Conversas</h1><p className="text-sm text-slate-500">Teste 7 · mensagens + dois Realtime</p></header>
