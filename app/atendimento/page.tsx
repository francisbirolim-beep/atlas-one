...
import { listarConversasAtendimento, listarMensagensAtendimento, buscarClienteAtendimento, type AtendimentoConversa, type AtendimentoCliente, type AtendimentoMensagem } from '@/lib/atendimento'
...
export default function AtendimentoPage(){
 ...
 useEffect(()=>{let ativo=true;(async()=>{try{const dados=await listarConversasAtendimento();if(!ativo)return;setConversas(dados);const ids=[...new Set(dados.map(x=>x.cliente_id).filter(Boolean))] as string[];const pares=await Promise.all(ids.map(async id=>[id,await buscarClienteAtendimento(id)] as const));if(ativo)setClientes(Object.fromEntries(pares))}catch(e){if(ativo)setErro(String(e))}})();return()=>{ativo=false}},[])
 useEffect(()=>{const conversa=conversas[0];if(!conversa)return;let ativo=true;listarMensagensAtendimento(conversa.id).then(m=>{if(ativo)setMensagens(m)}).catch(e=>{if(ativo)setErro(String(e))});setRt('Mensagens carregadas sem Realtime');return()=>{ativo=false}},[conversas[0]?.id])
 ...
 return <main className="min-h-screen bg-slate-100 p-3"><div className="mx-auto max-w-5xl overflow-hidden rounded-2xl border bg-white shadow-sm">
  <header className="border-b p-4"><h1 className="text-xl font-bold">Central de Conversas</h1><p className="text-sm text-slate-500">Teste 8 · mensagens sem Realtime</p></header>
