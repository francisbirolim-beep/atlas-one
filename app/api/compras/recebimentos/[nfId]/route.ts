...
async function carregarBase(nfId:string){
  const {data:nf,error:nfError}=await supabaseAdmin.from('compras_nfs').select('id,numero,serie,data_emissao,data_entrada,fornecedor_id,fornecedor_nome,fornecedor_cnpj,valor_total,status').eq('id',nfId).maybeSingle(); if(nfError)throw new Error(nfError.message); if(!nf)return null
  const {data:itens,error:itensError}=await supabaseAdmin.from('compras_nf_itens').select('id,produto_id,codigo_fornecedor,descricao,unidade,unidade_estoque,fator_conversao,quantidade,valor_unitario,valor_total,custo_aquisicao_unitario,vinculo_status').eq('nf_id',nfId).order('created_at',{ascending:true}); if(itensError)throw new Error(itensError.message)
  ...
  if(produtoIds.length){const {data:rows,error}=await supabaseAdmin.from('produtos').select('id,codigo,nome,unidade').in('id',produtoIds);if(error)throw new Error(error.message);for(const p of rows||[])produtos.set(p.id,p)}
  const {data:recebimentos,error:recebimentosError}=await supabaseAdmin.from('compras_recebimentos').select('id,status,data_recebimento,observacoes,recebido_por_nome,created_at').eq('nf_id',nfId).neq('status','cancelado').order('data_recebimento',{ascending:false});if(recebimentosError)throw new Error(recebimentosError.message)
  ...
  if(recebimentoIds.length){
    const {data:anteriores,error}=await supabaseAdmin.from('compras_recebimento_itens').select('nf_item_id,quantidade_recebida,quantidade_avariada').in('recebimento_id',recebimentoIds);if(error)throw new Error(error.message)
    ...
    const {data:fotos,error:fotosError}=await supabaseAdmin.from('compras_recebimento_fotos').select('id,recebimento_id,arquivo_nome,arquivo_path').in('recebimento_id',recebimentoIds).order('created_at',{ascending:true});if(fotosError)throw new Error(fotosError.message)
...
export async function POST(req:NextRequest,{params}:{params:{nfId:string}}){
  ...
  try{
    ...
    const {data:recebimento,error:recebimentoError}=await supabaseAdmin.from('compras_recebimentos').insert({nf_id:params.nfId,status:'concluido',data_recebimento:dataRecebimento,observacoes:texto(payload.observacoes,2000)||null,recebido_por_id:usuario.id,recebido_por_nome:usuario.nome}).select('id').single();if(recebimentoError)throw new Error(recebimentoError.message);recebimentoId=recebimento.id
    ...
    for(const foto of fotos){if(!foto.type.startsWith('image/'))throw new Error('A conferência aceita apenas imagens.');totalFotos+=foto.size;if(foto.size>2*1024*1024||totalFotos>5*1024*1024)throw new Error('As fotos excedem o limite permitido.');const caminho=`${usuario.id}/${new Date().toISOString().slice(0,7)}/${recebimento.id}/${randomUUID()}-${nomeSeguro(foto.name)}`;const buffer=Buffer.from(await foto.arrayBuffer());const {error}=await supabaseAdmin.storage.from('compras-recebimentos').upload(caminho,buffer,{contentType:foto.type||'image/jpeg',upsert:false});if(error)throw new Error(error.message);arquivosGuardados.push(caminho);registrosFotos.push({recebimento_id:recebimento.id,nf_item_id:null,arquivo_nome:foto.name,arquivo_path:caminho,mime_type:foto.type||null,criado_por_id:usuario.id,criado_por_nome:usuario.nome})}
  ...
  }catch(error){if(arquivosGuardados.length)try{await supabaseAdmin.storage.from('compras-recebimentos').remove(arquivosGuardados)}catch{};if(recebimentoId)try{await supabaseAdmin.from('compras_recebimentos').delete().eq('id',recebimentoId)}catch{};console.error('Erro ao registrar conferência:',error);return NextResponse.json({error:error instanceof Error?error.message:'Erro ao registrar conferência.'},{status:500})}
}
