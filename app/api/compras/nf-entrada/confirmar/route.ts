...
async function autenticar(req: NextRequest) {
  ...
  const { data: usuario } = await supabaseAdmin.from('usuarios').select('id,nome,role').eq('id', data.user.id).maybeSingle(); return usuario || null
}

async function resolverFornecedor(payload: PayloadEntrada, usuario: { id: string; nome: string }) {
  if (payload.fornecedorId) { const { data } = await supabaseAdmin.from('fornecedores').select('id,nome,cnpj_cpf').eq('id', payload.fornecedorId).maybeSingle(); if (data) return data }
  const cnpj = somenteDigitos(payload.fornecedorCnpj)
  if (cnpj) { const { data } = await supabaseAdmin.from('fornecedores').select('id,nome,cnpj_cpf').not('cnpj_cpf','is',null); const e = (data || []).find(i => somenteDigitos(i.cnpj_cpf) === cnpj); if (e) return e }
  const nome = texto(payload.fornecedorNome, 200); if (!nome && !cnpj) return null
  const { data, error } = await supabaseAdmin.from('fornecedores').insert({ nome: nome || `Fornecedor ${cnpj}`, cnpj_cpf: cnpj || null, ativo: true, criado_por_id: usuario.id, criado_por_nome: usuario.nome, observacoes: 'Criado a partir da confirmação de uma NF de entrada no Atlas.' }).select('id,nome,cnpj_cpf').single()
...
export async function POST(req: NextRequest) {
  const usuario = await autenticar(req); if (!usuario) return NextResponse.json({ error: 'Sessão inválida.' }, { status: 401 })
  ...
  try {
    ...
    const chave = somenteDigitos(payload.chaveAcesso).slice(0,44); if (chave) { const { data:d } = await supabaseAdmin.from('compras_nfs').select('id,numero').eq('chave_acesso',chave).maybeSingle(); if (d) return NextResponse.json({ error:`Esta NF-e já foi lançada no Atlas${d.numero?` (NF ${d.numero})`:''}.` },{status:409}) }
    ...
    if (produtoIds.length) { const { data,error } = await supabaseAdmin.from('produtos').select('id,custo,unidade').in('id',produtoIds); if(error) throw new Error(error.message); for(const p of data||[]) produtosMap.set(p.id,{id:p.id,custo:p.custo==null?null:Number(p.custo),unidade:p.unidade}); if(produtoIds.some(id=>!produtosMap.has(id))) throw new Error('Um ou mais produtos vinculados não existem mais no catálogo.') }

    const arquivo = form.get('arquivo'); let arquivoNome:string|null=null
    if (arquivo instanceof File && arquivo.size>0) { if(arquivo.size>15*1024*1024) return NextResponse.json({error:'O arquivo excede 15 MB.'},{status:413}); arquivoNome=arquivo.name; const caminho=`${usuario.id}/${new Date().toISOString().slice(0,7)}/${randomUUID()}-${nomeSeguro(arquivo.name)}`; const buffer=Buffer.from(await arquivo.arrayBuffer()); const {error}=await supabaseAdmin.storage.from('compras-nfs').upload(caminho,buffer,{contentType:arquivo.type||'application/octet-stream',upsert:false}); if(error) throw new Error(error.message); arquivoPath=caminho }

    const agora=new Date().toISOString(); const { data:nf,error:nfError }=await supabaseAdmin.from('compras_nfs').insert({
      origem_entrada:origem,status:'confirmada',chave_acesso:chave||null,numero:texto(payload.numero,40)||null,serie:texto(payload.serie,20)||null,data_emissao:payload.dataEmissao?new Date(payload.dataEmissao).toISOString():null,data_entrada:agora,
    ...
    if (fornecedor?.id) for (const item of itensValidos) { const pid=texto(item.produtoId,80); const cod=texto(item.codigoFornecedor,120); if(!pid||!cod) continue; await supabaseAdmin.from('produto_fornecedores').upsert({produto_id:pid,fornecedor_id:fornecedor.id,codigo_fornecedor:cod,descricao_fornecedor:item.descricao,ncm_fornecedor:texto(item.ncm,20)||null,unidade_compra:texto(item.unidade,30)||null,fator_conversao:numero(item.fatorConversao),preferencial:true,ativo:true,criado_por_id:usuario.id,criado_por_nome:usuario.nome,updated_at:agora},{onConflict:'fornecedor_id,codigo_fornecedor'}) }
    ...
    if(nfCriadaId){ try{await supabaseAdmin.from('financeiro_contas_pagar').delete().eq('nf_id',nfCriadaId);await supabaseAdmin.from('compras_nfs').delete().eq('id',nfCriadaId)}catch{} }
