...
async function validarDadosConfiguracao(body: any, excluirId?: string): Promise<{ dados?: DadosValidados; erro?: NextResponse }> {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const validacao = await validarDadosConfiguracao(body)
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const validacao = await validarDadosConfiguracao(body, id)
    if (validacao.erro) return validacao.erro
    const dados = validacao.dados!
    const agora = new Date().toISOString()
