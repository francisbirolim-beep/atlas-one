...
export async function GET(req:NextRequest){
 ...
 try{
  const {data:unidades,error}=await supabaseAdmin
   ...
   .select('id,codigo,nome,tipo,cidade,endereco,ativo,locais:estoque_locais(id,codigo,nome,tipo,permite_venda,ativo,enderecos:estoque_enderecos(id,codigo,zona,corredor,estante,prateleira,caixa,descricao,ativo))')
...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  if(acao==='unidade'){
   ...
   const {data,error}=await supabaseAdmin.from('unidades_operacionais').insert({codigo,nome,tipo,cidade:txt(b.cidade,120)||null,endereco:txt(b.endereco,240)||null,empresa_id:u.empresa_id}).select().single();if(error)throw error;return NextResponse.json({ok:true,unidade:data})
  }
  if(acao==='local'){
   ...
   const {data:unidade}=await supabaseAdmin.from('unidades_operacionais').select('id').eq('id',unidadeId).eq('empresa_id',u.empresa_id).maybeSingle()
   if(!unidade)return NextResponse.json({error:'Unidade inválida para esta empresa.'},{status:404})
   const {data,error}=await supabaseAdmin.from('estoque_locais').insert({unidade_id:unidadeId,codigo,nome,tipo,permite_venda:b.permiteVenda!==false,empresa_id:u.empresa_id}).select().single();if(error)throw error;return NextResponse.json({ok:true,local:data})
  }
  if(acao==='endereco'){
   ...
   const {data:local}=await supabaseAdmin.from('estoque_locais').select('id').eq('id',localId).eq('empresa_id',u.empresa_id).maybeSingle()
   if(!local)return NextResponse.json({error:'Local inválido para esta empresa.'},{status:404})
   const {data,error}=await supabaseAdmin.from('estoque_enderecos').insert({local_id:localId,codigo,zona:txt(b.zona,40)||null,corredor:txt(b.corredor,40)||null,estante:txt(b.estante,40)||null,prateleira:txt(b.prateleira,40)||null,caixa:txt(b.caixa,40)||null,descricao:txt(b.descricao,200)||null,empresa_id:u.empresa_id}).select().single();if(error)throw error;return NextResponse.json({ok:true,endereco:data})
  }
