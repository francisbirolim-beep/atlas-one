...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!unidade) {
      return NextResponse.json({ error: 'Informe a unidade operacional' }, { status: 400 })
    }
    if (unidade.length > 40) {
      return NextResponse.json({ error: 'A unidade operacional deve ter no máximo 40 caracteres' }, { status: 400 })
    }
    if (!evidencia) {
      return NextResponse.json({ error: 'Registre como a unidade foi confirmada' }, { status: 400 })
    }
    if (evidencia.length > 1000) {
      return NextResponse.json({ error: 'A evidência deve ter no máximo 1000 caracteres' }, { status: 400 })
    }
    ...
    if (erroUpdate) {
      return NextResponse.json({ error: 'Não foi possível registrar a unidade operacional' }, { status: 400 })
    }
    if (!atualizado) {
      return NextResponse.json({ error: 'O produto foi alterado por outro usuário. Recarregue a fila antes de continuar.' }, { status: 409 })
    }
