  ...
  try {
    ...
    if (acao === 'concluir_reembolso') {
      ...
      const { data, error } = await supabaseAdmin.rpc('concluir_reembolso_balcao', {
        ...
        p_movimentar_caixa: movimentarCaixa,
        p_caixa_id: caixaId,
    ...
    if (!['cancelamento_total', 'devolucao_parcial'].includes(tipo)) {
      return NextResponse.json({ error: 'Escolha cancelamento total ou devolução parcial.' }, { status: 400 })
    }
    ...
    const itens = Array.isArray(body.itens) ? body.itens.map((i: any) => ({
      ...
      localRetornoId: String(i.localRetornoId || '') || null,
    })).filter((i: any) => i.itemId && i.quantidade > 0) : []

    if (tipo === 'devolucao_parcial' && !itens.length) {
      return NextResponse.json({ error: 'Selecione ao menos um item e quantidade para devolver.' }, { status: 400 })
    }
    ...
    const { data, error } = await supabaseAdmin.rpc('processar_cancelamento_devolucao_balcao', {
      ...
      p_usuario_id: usuario.id,
      p_usuario_nome: usuario.nome,
      p_reembolsar_caixa: reembolsarCaixa,
      p_caixa_id: caixaId,
    })
    if (error) throw error
    return NextResponse.json(data || { ok: true })
