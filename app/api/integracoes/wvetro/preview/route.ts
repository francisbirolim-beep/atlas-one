...
function paresLinhaModelo(payload: unknown): TipologiaResumo[] {
  ...
  visitar(payload)
  return Array.from(encontrados.values()).sort(
    (a, b) => a.linha.localeCompare(b.linha, 'pt-BR') || a.modelo.localeCompare(b.modelo, 'pt-BR'),
  )
}
...
function resumoComponentes(payload: unknown) {
  ...
  const ordenar = (m: Map<string, ComponenteResumo>) =>
    Array.from(m.values()).sort((a, b) => a.codigo.localeCompare(b.codigo, 'pt-BR'))
...
function compararComponente(
  componente: ComponenteResumo,
  indice: Map<string, ProdutoAtlas[]>,
): ComponenteComparado {
  const chaves = Array.from(
    new Set(
      [normalizarCodigo(componente.codigo), normalizarCodigo(componente.codigoWvetro)].filter(Boolean),
    ),
  )
  ...
  if (
    (componente.custoMin !== null || componente.custoMax !== null) &&
    (custoAtlas === null || !dentroDaFaixa(custoAtlas, componente.custoMin, componente.custoMax))
  ) {
    motivos.push('Custo atual do Atlas está fora da faixa encontrada no histórico W.Vetro.')
  }

  if (
    (componente.vendaMin !== null || componente.vendaMax !== null) &&
    (precoAtlas === null || !dentroDaFaixa(precoAtlas, componente.vendaMin, componente.vendaMax))
  ) {
...
function compararTipologias(
  ...
  return tipologias.map(item => {
    ...
    const modelos = (tipologiasPorLinha.get(linha.id) || []).filter(
      t => normalizarTexto(modeloDoLabelAtlas(t.label)) === modeloNormalizado,
    )
...
function contarStatus<T extends { statusComparacao: StatusComparacao }>(itens: T[]) {
  return itens.reduce(
    ...
    { existente: 0, novo: 0, divergente: 0, linha_nao_mapeada: 0 } as Record<
      StatusComparacao,
      number
    >,
  )
...
async function buscarTodosProdutosAtlas(): Promise<ProdutoAtlas[]> {
  const tamanhoPagina = 1000
  const produtos: ProdutoAtlas[] = []
  let inicio = 0
  ...
  while (true) {
    const { data, error } = await supabaseAdmin
      .from('produtos')
      .select(
        'id,codigo,codigo_origem,id_externo_wvetro,nome,categoria,custo,preco,origem,status_validacao',
      )
      .order('id', { ascending: true })
      .range(inicio, inicio + tamanhoPagina - 1)

    if (error) {
      throw new Error(`Não foi possível carregar o catálogo completo do Atlas: ${error.message}`)
    }

    const pagina = (data || []) as ProdutoAtlas[]
...
async function reconciliarComAtlas(
  tipologias: TipologiaResumo[],
  componentes: ReturnType<typeof resumoComponentes>,
) {
  const [produtos, linhasResp, tipologiasResp, vinculosResp] = await Promise.all([
    buscarTodosProdutosAtlas(),
  ...
  const erro = linhasResp.error || tipologiasResp.error || vinculosResp.error
...
export async function GET(req: NextRequest) {
  ...
  if (recurso === 'status') {
    return NextResponse.json({ ok: true, modo: 'somente-leitura', configuracao: status })
  }
  ...
  try {
    ...
    if (recurso === 'produto') {
      ...
      if (!['A', 'P', 'E'].includes(tipo) || !codigo) {
        return NextResponse.json(
          { error: 'Informe tipo=A|P|E e codigo do produto.' },
          { status: 400 },
        )
      }
    ...
    if (['orcamentos', 'pedidos', 'resumo'].includes(recurso)) {
      ...
      if (dias < 0 || dias > 90) {
        return NextResponse.json(
          { error: 'O período de prévia deve ter entre 0 e 90 dias.' },
          { status: 400 },
        )
      }
      ...
      const fonte =
        recurso === 'resumo'
          ? String(req.nextUrl.searchParams.get('fonte') || 'orcamentos')
          : recurso

      if (!['orcamentos', 'pedidos'].includes(fonte)) {
        return NextResponse.json(
          { error: 'Fonte inválida. Use orcamentos ou pedidos.' },
          { status: 400 },
        )
      }
      ...
      return NextResponse.json({
        ...
        modo: 'somente-leitura',
        periodo: { inicio, fim },
        tipologiasEncontradas: tipologias,
        dados,
      })
    }

    return NextResponse.json(
      { error: 'Recurso inválido. Use status, linhas, produto, orcamentos, pedidos ou resumo.' },
      { status: 400 },
    )
  } catch (error) {
