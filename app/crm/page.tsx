...
export default function CRM() {
  ...
  if (melhorOportunidade) {
    insights.push({
      texto: `Maior oportunidade em aberto: ${melhorOportunidade.cliente_nome} — ${formatarMoeda(melhorOportunidade.valor_estimado || 0)}.`,
      tipo: 'oportunidade',
    })
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-brand-navyLight">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        {nivel === 'consulta' && <p className="text-xs text-slate-400">Seu acesso ao CRM é de somente consulta.</p>}

        <div className="grid sm:grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl border border-slate-200 p-5">
            <p className="text-xs text-slate-500 mb-1">Taxa de conversão</p>
            <p className="text-2xl font-bold text-slate-800">{formatarPercentual(taxaConversao)}</p>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5">
            ...
            {metaGeral.valor ? (
              <>
                <p className="text-2xl font-bold text-slate-800">{formatarMoeda(valorFechadoMes)} <span className="text-sm font-normal text-slate-400">/ {formatarMoeda(metaGeral.valor)}</span></p>
                <div className="w-full h-1.5 bg-slate-100 rounded-full mt-2 overflow-hidden">
                  <div className="h-full bg-brand-teal rounded-full" style={{ width: `${progressoValor}%` }} />
                </div>
              </>
            ) : (
              <p className="text-sm text-slate-400">Sem meta definida. {nivel === 'edicao' && <button onClick={() => setMetasAberto(true)} className="text-brand-navy hover:underline">Configurar</button>}</p>
            )}
          </div>
          <div className="bg-white rounded-2xl border border-slate-200 p-5">
            ...
            {metaGeral.quantidade ? (
              <>
                <p className="text-2xl font-bold text-slate-800">{formatarNumero(fechadosNoMes.length)} <span className="text-sm font-normal text-slate-400">/ {formatarNumero(metaGeral.quantidade)}</span></p>
                <div className="w-full h-1.5 bg-slate-100 rounded-full mt-2 overflow-hidden">
        ...
        {nivel === 'edicao' && metasAberto && (
          <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-3">
            <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-700"><Target size={16} /> Metas comerciais do mês</h2>
            <p className="text-xs text-slate-400">Meta de {new Date(mesMetaAtual + '-01T00:00:00').toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}. Vale como valor fechado (aprovado/convertido) e/ou quantidade de negócios fechados no mês.</p>
            <div className="space-y-2">
              <div className="border border-slate-100 rounded-xl p-3">
                <p className="text-sm font-medium text-slate-700 mb-2">Empresa toda</p>
                <div className="grid grid-cols-2 gap-3 mb-2">
                  <div><label className="block text-xs text-slate-500 mb-1">Meta em R$</label><input type="number" value={metas.geral?.valor ?? ''} onChange={e => setMetas(prev => ({ ...prev, geral: { ...(prev.geral || { valor: '', quantidade: '' }), valor: e.target.value } }))} placeholder="Ex: 50000" className="w-full border border-slate-300 rounded-lg p-2 text-sm" /></div>
                  <div><label className="block text-xs text-slate-500 mb-1">Meta em quantidade</label><input type="number" value={metas.geral?.quantidade ?? ''} onChange={e => setMetas(prev => ({ ...prev, geral: { ...(prev.geral || { valor: '', quantidade: '' }), quantidade: e.target.value } }))} placeholder="Ex: 10" className="w-full border border-slate-300 rounded-lg p-2 text-sm" /></div>
                </div>
                <button onClick={() => salvarMetaUsuario(null, null)} disabled={salvandoMeta === 'geral'} className="text-xs text-brand-navy hover:underline">{salvandoMeta === 'geral' ? 'Salvando...' : 'Salvar meta da empresa'}</button>
              </div>
              {usuariosMeta.map(u => (
                <div key={u.id} className="border border-slate-100 rounded-xl p-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-2">
                    <div><label className="block text-xs text-slate-500 mb-1">Meta em R$</label><input type="number" value={metas[u.id]?.valor ?? ''} onChange={e => setMetas(prev => ({ ...prev, [u.id]: { ...(prev[u.id] || { valor: '', quantidade: '' }), valor: e.target.value } }))} placeholder="Ex: 15000" className="w-full border border-slate-300 rounded-lg p-2 text-sm" /></div>
                    <div><label className="block text-xs text-slate-500 mb-1">Meta em quantidade</label><input type="number" value={metas[u.id]?.quantidade ?? ''} onChange={e => setMetas(prev => ({ ...prev, [u.id]: { ...(prev[u.id] || { valor: '', quantidade: '' }), quantidade: e.target.value } }))} placeholder="Ex: 3" className="w-full border border-slate-300 rounded-lg p-2 text-sm" /></div>
                  </div>
                  <button onClick={() => salvarMetaUsuario(u.id, u.nome)} disabled={salvandoMeta === u.id} className="text-xs text-brand-navy hover:underline">{salvandoMeta === u.id ? 'Salvando...' : 'Salvar meta'}</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-700 mb-1"><Sparkles size={16} className="text-brand-teal" /> IA Comercial</h2>
          <p className="text-xs text-slate-400 mb-3">Sugestões automáticas com base nos leads, tarefas e contatos registrados.</p>
          <div className="space-y-2">{insights.map((ins, i) => <div key={i} className={`text-sm px-3 py-2 rounded-lg ${ins.tipo === 'alerta' ? 'bg-red-50 text-red-700' : ins.tipo === 'oportunidade' ? 'bg-amber-50 text-amber-700' : 'bg-slate-50 text-slate-600'}`}>{ins.texto}</div>)}</div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            {STATUS_FUNIL.map(s => {
              ...
              return <div key={s.valor} className="border border-slate-100 rounded-xl p-3 text-center"><span className={`inline-block text-xs px-2 py-0.5 rounded-full font-medium mb-2 ${s.cor}`}>{s.label}</span><p className="text-xl font-bold text-slate-800">{formatarNumero(itens.length)}</p><p className="text-xs text-slate-400">{formatarMoeda(valor)}</p></div>
            })}
          </div>
        </div>
            ...
            {colunasTemperatura.map(col => {
              ...
              return (
                <div key={col.chave} className={`rounded-2xl border ${col.corBorda} bg-white overflow-hidden`}>
                  <div className={`${col.corHeader} px-4 py-3 flex items-center justify-between`}><span className={`font-semibold text-sm ${col.corTexto}`}>{col.emoji} {col.label}</span><span className={`text-xs font-medium ${col.corTexto}`}>{formatarNumero(cardsColuna.length)}</span></div>
                  <div className="p-3 space-y-2 min-h-[100px] max-h-80 overflow-y-auto">
                    {cardsColuna.length === 0 && <p className="text-xs text-slate-300 text-center py-6">Nenhum orçamento aqui</p>}
                    {cardsColuna.map(card => (
                      <Link key={card.id} href={`/kanban?orcamento=${card.id}`} className="block rounded-xl border border-slate-200 p-3 hover:border-slate-300 hover:shadow-sm transition">
                        <p className="font-medium text-sm text-slate-800 truncate">{card.cliente_nome}</p>
                        {card.cidade && <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5"><MapPin size={11} /> {card.cidade}</p>}
                        {card.criado_por_nome && <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5"><User size={11} /> {card.criado_por_nome}</p>}
                        {card.valor_estimado != null && <p className="text-xs font-semibold text-emerald-600 mt-1">{formatarMoeda(card.valor_estimado)}</p>}
                      </Link>
                    ))}
                  </div>
                </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-700 mb-3"><CheckSquare size={16} /> Tarefas e retornos pendentes</h2>
          {tarefas.length === 0 ? <p className="text-sm text-slate-400">Nenhuma tarefa pendente. Crie tarefas na página de cada cliente.</p> : (
            <div className="space-y-1.5">
              {tarefas.slice(0, 20).map(t => {
                const atrasada = t.data_vencimento && t.data_vencimento < hojeStr
                return <div key={t.id} className="flex items-center gap-2 py-1.5 border-b border-slate-100 last:border-0"><button onClick={() => alternarTarefa(t)} className="text-slate-400 hover:text-brand-navy flex-shrink-0"><CheckSquare size={16} className="opacity-30 hover:opacity-100" /></button><div className="flex-1 min-w-0"><p className="text-sm text-slate-700 truncate">{t.titulo}{t.cliente_nome && <span className="text-slate-400"> · {t.cliente_nome}</span>}</p></div>{t.data_vencimento && <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${atrasada ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-500'}`}>{new Date(t.data_vencimento + 'T00:00:00').toLocaleDateString('pt-BR')}</span>}{t.cliente_id && <Link href={`/clientes/${t.cliente_id}`} className="text-xs text-brand-navy hover:underline flex-shrink-0">Ver cliente</Link>}</div>
              })}
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-700 mb-3"><Send size={16} /> Propostas enviadas — aguardando resposta</h2>
          {propostasEnviadas.length === 0 ? <p className="text-sm text-slate-400">Nenhuma proposta aguardando resposta no momento.</p> : (
            <div className="space-y-1.5">{propostasEnviadas.map(c => <Link key={c.id} href={c.cliente_id ? `/clientes/${c.cliente_id}` : `/kanban?orcamento=${c.id}`} className="flex items-center justify-between gap-2 py-2 border-b border-slate-100 last:border-0 hover:bg-slate-50 -mx-2 px-2 rounded-lg transition"><div className="flex items-center gap-2 min-w-0">{c.temperatura === 'quente' && <Flame size={13} className="text-red-500 flex-shrink-0" />}<p className="text-sm text-slate-700 truncate">{c.cliente_nome}</p></div><p className="text-sm font-semibold text-emerald-600 flex-shrink-0">{c.valor_estimado != null ? formatarMoeda(c.valor_estimado) : ''}</p></Link>)}</div>
          )}
        </div>
      </main>
    </div>
