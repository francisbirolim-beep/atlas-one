...
export default function ConfiguracoesOrcamentoPage() {
  ...
  function cancelarEdicao() {
    limparCampos()
    setErro('')
    setSucesso('Edição cancelada. Nenhuma alteração foi salva.')
  }
  ...
  async function salvar() {
    ...
    setSucesso(estavaEditando
      ? 'Configuração atualizada sem criar duplicata.'
      : 'Configuração validada e publicada para o orçamento.')
  }
  ...
  async function alternar(item: ConfiguracaoOrcamento) {
    ...
    if (!resultado.ok) {
      setErro(resultado.error || 'Não foi possível alterar a configuração.')
      return
    }
    setConfiguracoes(await listarConfiguracoesOrcamentoAdministracao())
  }
  ...
  if (carregando) {
    return <div className="min-h-screen grid place-items-center text-slate-400"><Loader2 className="animate-spin" /></div>
  }
    ...
    return (
      <div className="min-h-screen grid place-items-center px-4">
        <div className="max-w-md text-center">
          ...
          <p className="text-slate-600">Somente o Master pode publicar configurações validadas para o orçamento.</p>
          <Link href="/engenharia" className="text-brand-navy text-sm hover:underline">Voltar à Engenharia</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-brand-navyLight">
      <header className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-slate-800">Configurações validadas de orçamento</h1>
            <p className="text-sm text-slate-500">Modelos rápidos: Linha → Tipologia → combinação técnica conferida</p>
          </div>
        </div>
      </header>

      <main className="max-w-6xl mx-auto px-4 py-7 grid lg:grid-cols-[420px_1fr] gap-6">
        <section className={`bg-white border rounded-2xl p-5 space-y-4 h-fit ${editandoId ? 'border-brand-navy/30 ring-2 ring-brand-navy/5' : 'border-slate-200'}`}>
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="font-semibold text-slate-800">{editandoId ? 'Editar configuração validada' : 'Nova configuração validada'}</h2>
              <p className="text-xs text-slate-500 mt-1">
                {editandoId
                  ? 'As alterações serão salvas no mesmo preset, sem criar outra configuração.'
                  : 'Só publique combinações já conferidas. O vendedor poderá escolhê-las com um clique.'}
              </p>
            </div>
            {editandoId && (
              <button onClick={cancelarEdicao} className="text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50 flex items-center gap-1">
          ...
          <div>
            <label className="text-xs text-slate-500">Linha (opcional, usada para filtrar)</label>
            <select
              value={linhaId}
              disabled={Boolean(editandoId)}
              onChange={e => { setLinhaId(e.target.value); setTipologiaId(''); setProdutoId('') }}
              className="w-full mt-1 border border-slate-300 rounded-xl p-2.5 text-sm disabled:bg-slate-100 disabled:text-slate-500"
            >
          ...
          <div>
            <label className="text-xs text-slate-500">Tipologia *</label>
            <select
              value={tipologiaId}
              disabled={Boolean(editandoId)}
            ...
            >
              ...
              {tipologiasFiltradas.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
            </select>
            {editandoId && <p className="mt-1 text-[11px] text-slate-400">Linha e tipologia ficam bloqueadas durante a edição para preservar a identidade técnica do preset.</p>}
          </div>

          <div>
            <label className="text-xs text-slate-500">Produto base (opcional)</label>
            <select value={produtoId} onChange={e => setProdutoId(e.target.value)} className="w-full mt-1 border border-slate-300 rounded-xl p-2.5 text-sm">
          ...
          <div>
            <label className="text-xs text-slate-500">Nome que o vendedor verá *</label>
            <input value={nome} onChange={e => setNome(e.target.value)} placeholder="Ex: JC3 — vidro + persiana + vidro" className="w-full mt-1 border border-slate-300 rounded-xl p-2.5 text-sm"/>
          </div>
          ...
          {tipologiaId && (
            <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              {variaveis.map(v => (
                <div key={v.id}>
                  <label className="text-xs text-slate-500">{v.variavel.label}{v.obrigatorio ? ' *' : ''}</label>
                  <select
                    value={valores[v.variavel.chave] || ''}
                    onChange={e => setValores(prev => ({ ...prev, [v.variavel.chave]: e.target.value }))}
                    className="w-full mt-1 border border-slate-300 rounded-lg p-2 text-sm"
                  >
                    <option value="">A definir</option>
                    {opcoesDaVariavel(v.variavel_id).map(o => <option key={o.id} value={o.chave}>{o.label}</option>)}
                  </select>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <label className="mt-1 flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-3 text-sm text-slate-600 hover:border-brand-navy">
              <Upload size={16}/>
              {imagemArquivo || imagemExistenteUrl ? 'Trocar imagem' : 'Selecionar imagem'}
              <input type="file" accept="image/*" className="hidden" onChange={e => setImagemArquivo(e.target.files?.[0] || null)}/>
            </label>
            {imagemExibida && (
              <div className="mt-2 overflow-hidden rounded-xl border border-slate-200 bg-white">
                <img src={imagemExibida} alt="Prévia da configuração" className="h-40 w-full object-contain"/>
              </div>
            )}
            {(imagemArquivo || imagemExistenteUrl) && (
              <button
                type="button"
                onClick={() => { setImagemArquivo(null); setImagemExistenteUrl('') }}
                className="mt-2 text-xs text-red-600 hover:underline"
              >
                Remover imagem desta configuração
              </button>
            )}
          ...
          <div>
            <label className="text-xs text-slate-500">Evidência técnica da validação *</label>
            <textarea value={evidencia} onChange={e => setEvidencia(e.target.value)} placeholder="Ex: conferido no relatório W.Vetro OC..., catálogo..., teste de produção..." className="w-full mt-1 border border-slate-300 rounded-xl p-2.5 text-sm h-24 resize-none"/>
          </div>

          <button onClick={salvar} disabled={salvando} className="w-full flex items-center justify-center gap-2 rounded-xl bg-brand-navy text-white py-3 text-sm font-semibold disabled:opacity-50">
            <Save size={16}/>{salvando ? 'Salvando...' : editandoId ? 'Salvar alterações' : 'Salvar e validar'}
          </button>
        </section>
        ...
        <section className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-4">
            <div className="relative">
              ...
              <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar configuração, tipologia, produto ou variável..." className="w-full pl-9 pr-3 py-2.5 border border-slate-200 rounded-xl text-sm"/>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            {configuracoesFiltradas.map(c => {
              ...
              return (
                <article key={c.id} className={`bg-white border rounded-2xl p-4 ${c.ativo === false ? 'border-slate-200 opacity-60' : c.validado ? 'border-emerald-200' : 'border-amber-200'}`}>
                  <div className="flex items-start gap-3">
                    <div className="h-24 w-28 shrink-0 overflow-hidden rounded-xl border border-slate-100 bg-slate-50 flex items-center justify-center">
                      {c.imagem_url || p?.foto_url
                        ? <img src={c.imagem_url || p?.foto_url || ''} alt={c.nome} className="h-full w-full object-contain"/>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="flex gap-2 flex-wrap">
                            <span className="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-600">{t?.label || 'Tipologia'}</span>
                            {c.validado && <span className="text-xs px-2 py-1 rounded-full bg-emerald-50 text-emerald-700 flex items-center gap-1"><ShieldCheck size={12}/> Validada</span>}
                          </div>
                        ...
                        <div className="flex flex-col gap-2 shrink-0">
                          <button onClick={() => iniciarEdicao(c)} className="text-xs px-2.5 py-1.5 rounded-lg border border-brand-navy/20 text-brand-navy hover:bg-brand-navyLight flex gap-1 items-center">
                            <Pencil size={12}/> Editar
                          </button>
                          <button onClick={() => alternar(c)} className={`text-xs px-2.5 py-1.5 rounded-lg border ${c.ativo === false ? 'border-emerald-200 text-emerald-700' : 'border-red-200 text-red-600'}`}>
                            {c.ativo === false
                              ? <span className="flex gap-1 items-center"><CheckCircle2 size={12}/> Ativar</span>
                              : <span className="flex gap-1 items-center"><XCircle size={12}/> Desativar</span>}
                          </button>
                        </div>
                      </div>
                    </div>
