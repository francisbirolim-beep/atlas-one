...
export default function OrcamentoRapido() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-brand-navyLight">
      ...
      {conferenciaAberta && <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/55 p-3 sm:p-4"><div className="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl sm:max-h-[calc(100dvh-2rem)]"><div className="flex shrink-0 items-start justify-between border-b px-4 py-3 sm:px-5 sm:py-4"><div><h2 className="text-lg font-bold text-slate-800">Conferência final do orçamento</h2><p className="mt-1 text-sm text-slate-500">Confira cada esquadria e o tipo de medida.</p></div><button onClick={() => setConferenciaAberta(false)}><X size={18}/></button></div><div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-3 sm:px-5 sm:py-4"><div className="mb-4 grid gap-3 rounded-xl border bg-slate-50 p-4 sm:grid-cols-3"><div><p className="text-[11px] uppercase text-slate-400">Cliente</p><p className="font-semibold">{clienteNome}</p></div><div><p className="text-[11px] uppercase text-slate-400">Cidade</p><p className="font-semibold">{cidade}</p></div><div><p className="text-[11px] uppercase text-slate-400">Resumo</p><p className="font-semibold">{itens.length} {itens.length===1?'item':'itens'} • {totalEsquadrias} esquadria(s)</p></div></div><div className="space-y-3">{itens.map((item,idx) => <div key={item.id} className="rounded-xl border-2 border-slate-200 p-4"><div className="flex justify-between gap-3"><div><p className="text-xs uppercase text-slate-400">Item {idx+1}</p><p className="font-bold">{item.quantidade || '1'}x {nomeTipologia(item)}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-medium ${item.tipoMedida === 'final' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{item.tipoMedida === 'final' ? 'Medida final' : 'Medida comum'}</span></div><div className="mt-3 grid gap-2 text-sm text-slate-600 sm:grid-cols-2"><p><b>Medidas:</b> {resumoMedidas(item)}</p><p><b>Linha:</b> {item.linhaNome || 'Não informada'}</p><p><b>Folhas:</b> {item.folhas || 'Não informado'}</p><p><b>Cor:</b> {item.cor || (acabamento === 'outro' ? acabamentoOutroTexto : acabamento) || 'Não informada'}</p></div>{item.ambiente && <p className="mt-2 text-xs"><b>Ambiente:</b> {item.ambiente}</p>}{item.descricao && <p className="mt-2 rounded-lg bg-slate-50 px-3 py-2 text-xs"><b>Observação:</b> {item.descricao}</p>}</div>)}</div></div><div className="grid shrink-0 grid-cols-2 gap-2 border-t bg-slate-50 px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 sm:gap-3 sm:px-5 sm:py-4"><button onClick={() => setConferenciaAberta(false)} className="rounded-xl border bg-white px-2 py-3 text-sm font-semibold sm:px-4">Voltar e corrigir</button><button onClick={() => void confirmarEnvio()} disabled={salvando} className="flex items-center justify-center gap-2 rounded-xl bg-brand-teal px-2 py-3 text-sm font-semibold text-white sm:px-4"><Send size={17}/>{salvando?'Enviando...':'Confirmar e enviar'}</button></div></div></div>}
