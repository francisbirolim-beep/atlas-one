...
export default function VendaBalcaoPage(){
 ...
 return <main className="min-h-screen bg-slate-50 p-3"><div className="mx-auto max-w-[1540px] space-y-2.5">
  ...
  <section className="rounded-xl border bg-white p-2.5"><div className="grid gap-2 lg:grid-cols-[1fr_auto]"><div className="relative"><Search size={15} className="absolute left-3 top-2.5 text-slate-400"/><input value={cliente?cliente.nome:clienteBusca} onInput={e=>atualizarClienteBusca(e.currentTarget.value)} onCompositionUpdate={e=>atualizarClienteBusca(e.currentTarget.value)} placeholder="Cliente obrigatório — nome, CPF/CNPJ ou telefone" className="w-full rounded-lg border py-2 pl-9 pr-3 text-xs"/>{!cliente&&carregandoClientes&&clienteBusca.trim().length>=2&&<Loader2 size={13} className="absolute right-3 top-2.5 animate-spin text-slate-400"/>}{!cliente&&clientes.length>0&&<div className="absolute z-20 mt-1 max-h-44 w-full overflow-auto rounded-lg border bg-white shadow-lg">{clientes.map(c=><button key={c.id} onClick={()=>{setCliente(c);setClienteBusca(c.nome);setClientes([])}} className="block w-full border-b px-3 py-2 text-left text-xs hover:bg-slate-50"><strong>{c.nome}</strong><div className="text-[10px] text-slate-400">{[c.cpf_cnpj,c.whatsapp||c.telefone,c.cidade].filter(Boolean).join(' • ')||'Sem telefone'}</div></button>)}</div>}</div><Link href="/balcao/clientes/novo" className="inline-flex items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-semibold"><UserPlus size={14}/>Novo cliente</Link></div><p className="mt-2 text-[10px] text-slate-500">Para finalizar: nome e telefone/WhatsApp obrigatórios. A venda fica registrada no Cliente 360.</p></section>
