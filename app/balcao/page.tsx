...
export default function VendaBalcaoPage(){
 ...
 useEffect(()=>{
  ...
  if(!clienteId)return
  let ativo=true
  ;(async()=>{try{const r=await api(`/api/balcao/catalogo?tipo=clientes&clienteId=${encodeURIComponent(clienteId)}`);const j=await r.json();const c=(j.clientes||[])[0] as Cliente|undefined;if(ativo&&r.ok&&c){setCliente(c);setClienteBusca(c.nome);setClientes([])}}catch{}})()
  return()=>{ativo=false}
 },[])
 useEffect(()=>{
 ...
 async function finalizar(){setErro('');setSucesso(null);if(!caixa)return setErro('Abra o caixa antes de finalizar a venda.');if(!itens.length)return setErro('Adicione pelo menos um produto.');if(Math.abs(falta)>0.01)return setErro(`Os pagamentos precisam fechar o total. Diferença: ${moeda(falta)}`);for(const p of pagamentos)if((p.forma==='boleto'||p.forma==='a_prazo')&&!p.primeiroVencimento)return setErro('Informe o primeiro vencimento do boleto/venda a prazo.');setSalvando(true);try{const params=new URLSearchParams(window.location.search);const clienteContexto=params.get('cliente');const obraContexto=params.get('obra');const resp=await api('/api/balcao/vendas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({clienteId:cliente?.id||clienteContexto||null,clienteNome:cliente?.nome||null,obraId:obraContexto||null,desconto:descontoN,itens:itens.map(i=>({produtoId:i.id,quantidade:i.quantidade,precoUnitario:i.precoUnitario,localOrigemId:i.localOrigemId,atendimento:i.atendimento})),pagamentos:pagamentos.map(p=>({forma:p.forma,valor:num(p.valor),parcelas:p.parcelas,primeiroVencimento:p.primeiroVencimento,intervaloDias:p.intervaloDias}))})});const json=await resp.json();if(!resp.ok)throw new Error(json.error||'Não foi possível finalizar.');setSucesso({numero:Number(json.numero),total:Number(json.total),atendimentoStatus:json.atendimentoStatus,reservasPendentes:Number(json.reservasPendentes||0)});setCarrinho({});setDesconto('0');setPagamentos([novoPagamento()]);if(!clienteContexto){setCliente(null);setClienteBusca('')}if(busca.trim())await carregarProdutos(busca);await carregarCaixa()}catch(e){setErro(e instanceof Error?e.message:'Erro ao finalizar a venda.')}finally{setSalvando(false)}}
