...
// Fase 8: quem criou o orçamento pode abrir e editar (ou acrescentar itens)
// direto, sem precisar que alguém clique em "Iniciar orçamento" primeiro —
// isso continua exigido só para os outros usuários.
const souCriador = !!usuario && !!cardSelecionado && !!cardSelecionado.criado_por_id && usuario.id === cardSelecionado.criado_por_id
const naPrimeiraColuna = !!editando && (editando.coluna_id || colunas[0]?.id) === colunas[0]?.id
const podeEditarSemIniciar = souCriador && naPrimeiraColuna && !sessaoAtiva && !editando?.orcamento_iniciado_em
...
<p className="text-xs text-brand-navy bg-brand-navyLight rounded-lg p-2.5">
Esse é o pedido que você mandou. Você pode corrigir os dados ou acrescentar/tirar esquadrias antes que alguém comece a orçar.
</p>
)}
