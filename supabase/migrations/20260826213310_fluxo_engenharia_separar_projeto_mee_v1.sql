-- Corrige a automação legada da Medição Final para apontar explicitamente ao MEE.
-- Antes ela buscava qualquer setor com "Engenharia" no nome e passou a colidir
-- com o novo setor pré-medição "Engenharia — Conferir Projeto".
...
begin
  ...
  if not exists (select 1 from public.setores s where s.id = v_setor_id) then
    raise exception 'Setor MEE não encontrado no cadastro de setores';
  end if;
  ...
  if v_item_existente is not null then
    update public.setor_kanban_itens
       set titulo = new.cliente_nome,
           ...
           atualizado_em = now(),
           atualizado_por_id = new.aprovado_por_id,
           atualizado_por_nome = new.aprovado_por_nome
...
revoke execute on function public.fn_medicao_aprovada_para_engenharia() from public, anon, authenticated;
