...
set search_path=''
as $$
declare v_empresa_assistencia uuid;
begin
  select a.empresa_id into v_empresa_assistencia from public.assistencias a where a.id = new.assistencia_id;
  if v_empresa_assistencia is null then raise exception 'Assistência sem empresa válida'; end if;
  if new.empresa_id is distinct from v_empresa_assistencia then raise exception 'Acesso externo pertence a outra empresa'; end if;
  return new;
end;
$$;
...
create trigger trg_assistencia_acesso_externo_empresa_guard before insert or update on public.assistencia_acessos_externos for each row execute function private.guard_assistencia_acesso_externo_empresa();
commit;
