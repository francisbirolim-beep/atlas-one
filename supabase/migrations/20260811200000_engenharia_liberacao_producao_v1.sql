...
begin
  ...
  if lower(v_destino_nome) like '%liberad%produ%' then
    ...
    select mf.id into v_medicao_id from public.medicoes_finais mf
    ...
    select count(*) into v_total_conferidos from public.medicao_itens mi
    ...
    -- Serializa liberacoes concorrentes do mesmo orcamento sem exigir limpeza previa de dados legados.
    ...
    if v_producao_coluna_id is null then
      insert into public.producao_colunas (nome, ordem) values ('Medição final', 0) returning id into v_producao_coluna_id;
    end if;

    select id into v_producao_item_id from public.producao_itens
     where orcamento_id = v_card.orcamento_id order by created_at asc limit 1 for update;
    ...
    if v_producao_item_id is null then
      ...
      update public.producao_itens
       ...
       where id = v_producao_item_id;
    end if;

    update public.setor_kanban_itens
       set coluna_id = p_coluna_id, atualizado_em = now(), liberado_producao_em = now(),
           liberado_producao_por_id = p_usuario_id, liberado_producao_por_nome = nullif(p_usuario_nome, '')
     where id = p_card_id;
  else
    update public.setor_kanban_itens set coluna_id = p_coluna_id, atualizado_em = now() where id = p_card_id;
  end if;
end;
$$;
