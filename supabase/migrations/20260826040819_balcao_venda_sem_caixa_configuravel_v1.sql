...
begin
  ...
  if v_local_padrao_id is null then
    select l.id, l.unidade_id into v_local_padrao_id, v_unidade_padrao_id
    from public.estoque_locais l
    join public.unidades_operacionais u on u.id=l.unidade_id
    where l.ativo=true and l.permite_venda=true
    order by case when u.codigo='MATRIZ' and l.codigo='GERAL' then 0 else 1 end, l.created_at
    limit 1;
  else
    select unidade_id into v_unidade_padrao_id
    from public.estoque_locais
    where id=v_local_padrao_id and ativo=true and permite_venda=true;
  end if;

  if v_local_padrao_id is null or v_unidade_padrao_id is null then
    raise exception 'Nenhum local de estoque disponível para a venda.';
  end if;

  for v_item in select * from jsonb_array_elements(p_itens) loop
    v_qtd:=nullif(v_item->>'quantidade','')::numeric;
    v_preco:=nullif(v_item->>'precoUnitario','')::numeric;
    v_local_origem_id:=coalesce(nullif(v_item->>'localOrigemId','')::uuid,v_local_padrao_id);
    ...
    select id,codigo,nome,unidade,custo,preco,preco_minimo,permite_venda_sem_estoque
      into v_prod from public.produtos
      where id=(v_item->>'produtoId')::uuid and ativo=true;
  ...
  insert into public.balcao_vendas(
    ...
    p_cliente_id,nullif(trim(p_cliente_nome),''),p_usuario_id,p_usuario_nome,null,
    v_unidade_padrao_id,v_local_padrao_id,round(v_subtotal,2),coalesce(p_desconto,0),v_total,
    concat_ws(E'\n',nullif(trim(p_observacoes),''),'Venda finalizada sem caixa aberto.'),'entregue'
        ...
        loop
          ...
          if v_local_origem_id=v_local_padrao_id then
            ...
            insert into public.estoque_movimentos(
              ...
              'venda_balcao',v_venda_id,'Venda balcão #'||v_numero||' sem caixa',p_usuario_id,p_usuario_nome,
  ...
  for v_pag in select * from jsonb_array_elements(p_pagamentos) loop
    ...
    insert into public.balcao_pagamentos(venda_id,forma,valor,parcelas,detalhes)
      values(v_venda_id,v_forma,v_valor,v_parcelas,concat_ws(' · ',nullif(v_pag->>'detalhes',''),'Sem caixa aberto'));
    ...
    if v_forma in ('boleto','a_prazo') then
      ...
      for i in 1..v_parcelas loop
        insert into public.financeiro_contas_receber(
          ...
          v_forma,'Gerado pela venda balcão #'||v_numero||' sem caixa',p_usuario_id,p_usuario_nome
        );
      end loop;
    end if;
  end loop;
  ...
  return jsonb_build_object(
    ...
    'itensAguardandoEstoque',v_itens_aguardando_estoque,'semCaixa',true
  );
end;
$function$;
