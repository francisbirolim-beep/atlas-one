alter table public.cadastro_historico drop constraint if exists cadastro_historico_tenant_guard;
alter table public.cadastro_historico
  add constraint cadastro_historico_tenant_guard
  check (
    empresa_id is not null
    or entidade_tabela in ('cores','engenharia_tipologia_variaveis','linhas','linhas_tecnicas','tipologia_campos_extras','tipologias')
  ) not valid;
...
begin
  ...
  if v_empresa_id is null then
    ...
    select coalesce(max(versao),0)+1 into v_versao
      from public.cadastro_historico
     where entidade_tabela=tg_table_name and entidade_id=v_id and empresa_id=v_empresa_id;
  end if;
