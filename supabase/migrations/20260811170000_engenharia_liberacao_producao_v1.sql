...
begin
  ...
  if not found then
    raise exception 'Obra da Engenharia nao encontrada';
  end if;
  ...
  if v_destino_nome is null then
    raise exception 'Etapa de destino da Engenharia nao encontrada';
  end if;
  ...
  if lower(v_destino_nome) like '%liberad%produ%' then
    ...
    select mf.id into v_medicao_id
     ...
     where mf.orcamento_id = v_card.orcamento_id
       and mf.status_operacional = 'aprovado'
     order by mf.aprovado_em desc nulls last
     limit 1;

    if v_medicao_id is null then
      raise exception 'Medicao Final aprovada nao encontrada para esta obra';
    end if;
    ...
    if v_producao_coluna_id is null then
      insert into public.producao_colunas (nome, ordem)
      values ('Medição final', 0)
      returning id into v_producao_coluna_id;
    end if;

    insert into public.producao_itens (
      titulo, descricao, coluna_id, orcamento_id,
      criado_por_id, criado_por_nome, atualizado_em
    ) values (
      v_card.titulo,
      v_card.descricao,
      v_producao_coluna_id,
      v_card.orcamento_id,
      p_usuario_id,
      coalesce(nullif(p_usuario_nome, ''), 'Engenharia'),
      now()
    )
